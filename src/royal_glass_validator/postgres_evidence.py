...
class PostgresEvidenceRepository:
    ...
    def record_evidence(self, evidence: PageEvidence) -> None:
        # Each page is a durable unit of work so an interrupted batch can resume
        # from retained evidence rather than silently losing prior pages.
    ...
    def record_failure(self, failure: FetchFailure) -> None:
        # Retain each failure immediately: unresolved fetches are an explicit
        # review-required result, not work that can disappear with the batch.
        with self._connection.transaction():
            with self._connection.cursor() as cursor:
                cursor.execute(
                    """
    ...
    def load_unfetched_selected_source_records(
        ...
        """Resume only selected records that have no durable evidence or failure yet."""
        ...
        with self._connection.cursor() as cursor:
            cursor.execute(
                ...
                SELECT source.id, source.validation_run_id, source.original_values
                 ...
                 ORDER BY source.input_row_number
                """,
                (validation_run_id, list(source_record_ids)),
            )
            rows = cursor.fetchall()
        return tuple(
            FetchSourceRecord(id=source_record_id, validation_run_id=run_id, original_values=original_values)
            for source_record_id, run_id, original_values in rows
        )
